...
def model_testing_score_evaluation(x_test, y_test):
    ...
    classifier_lr = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_logistic_regression.sav', 'rb'))
    result = str(classifier_lr.score(x_test, y_test) * 100) + "%"
    print(f'LOGISTIC REGRESSION : {result}\n')
    
    classifier_svc = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_support_vector_machine.sav', 'rb'))
    result = str(classifier_svc.score(x_test, y_test) * 100) + "%"
    print(f'SUPPORT VECTOR MACHINE : {result}\n')
    
    classifier_dt = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_decission_tree.sav', 'rb'))
    result = str(classifier_dt.score(x_test, y_test) * 100) + "%"
    print(f'DECISSION TREE : {result}\n')
    
    classifier_rf = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_random_forest.sav', 'rb'))
    result = str(classifier_rf.score(x_test, y_test) * 100) + "%"
    print(f'RANDOM FPREST : {result}\n')
    
    classifier_knn = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_k_nearest_neighbour.sav', 'rb'))
    result = str(classifier_knn.score(x_test, y_test) * 100) + "%"
    print(f'K NEAREST NEIGHBOUR : {result}\n')
    
    classifier_gnb = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_gaussian_naive_bayes.sav', 'rb'))
    result = str(classifier_gnb.score(x_test, y_test) * 100) + "%"
    print(f'GAUSSIAN NAIVE BAYES : {result}\n')
    
    classifier_lda = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_linear_discriminant_analysis.sav', 'rb'))
    result = str(classifier_lda.score(x_test, y_test) * 100) + "%"
    print(f'LINEAR DISCRIMINANT ANALYSIS : {result}\n')
    
    classifier_qda = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_quadratic_discriminant_analysis.sav', 'rb'))
    result = str(classifier_qda.score(x_test, y_test) * 100) + "%"
    print(f'QUADRATIC DISCRIMINANT ANALYSIS : {result}\n')
    
    classifier_adb = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_adaboost_classifier.sav', 'rb'))
    result = str(classifier_adb.score(x_test, y_test) * 100) + "%"
    print(f'ADABOOST : {result}\n')
    
    classifier_gbc = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_gradient_boosting_classifier.sav', 'rb'))
    result = str(classifier_gbc.score(x_test, y_test) * 100) + "%"
    print(f'GRADIENT BOOSTING CLASSIFIER : {result}\n')
    
    classifier_mlp = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_multilayer_perceptron.sav', 'rb'))
    result = str(classifier_mlp.score(x_test, y_test) * 100) + "%"
    print(f'MULTI LAYER PERCEPTRON : {result}\n')
    
    classifier_cbc = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_categorial_boosting_classifier.sav', 'rb'))
    result = str(classifier_cbc.score(x_test, y_test) * 100) + "%"
    print(f'CATEGORIAL BOOSTING CLASSIFIER : {result}\n')
    
    classsifier_xgbc = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_xgboost_classifier.sav', 'rb'))
    result = str(classsifier_xgbc.score(x_test, y_test) * 100) + "%"
    print(f'XGBOOST CLASSIFIER : {result}\n')
    
    classsifier_lmg = pickle.load(open('F:/Python_VS_Code/heart_disease_project/trained_models/trained_lightgbm_classifier.sav', 'rb'))
